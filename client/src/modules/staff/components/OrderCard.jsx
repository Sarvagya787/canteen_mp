    ...
    <div className="bg-white rounded-[1.5rem] border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col h-full">
      ...
      <div className="p-3 border-t border-slate-100 grid grid-cols-2 gap-3 bg-slate-50 rounded-b-[1.5rem]">
        ...
        <button
         ...
         onClick={
            order.fullfillment_status=='PENDING'? ()=>{processOrder(order._id.toString(),"PREPARING")}:
             order.fullfillment_status=="PREPARING"?()=>{processOrder(order._id.toString(),"READY")}:()=>{}
         }
        className="flex items-center justify-center gap-2 bg-slate-900 text-white py-2.5 rounded-xl text-xs font-bold hover:bg-green-600 transition shadow-lg shadow-slate-900/20">
            <IoCheckmarkCircle className="text-lg"/> {order.fullfillment_status=='PENDING'?"Start Preparing":order.fullfillment_status=="PREPARING"?"Mark As Ready":"NULL"}
        </button>
      </div>
